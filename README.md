...
### Quick Start
 ...
 2. **Launch the Application**: Execute the following command in your terminal:
   ...
   ```
   *(Note: The Gemini API key is already integrated into the environment secrets; no additional configuration is required from your side)*.
## 🧠 Key Design Decisions
 * **Sequential Orchestration**: I chose a sequential flow (Evaluator → Interviewer) to ensure every question is grounded in an objective analysis of the candidate's last answer, fulfilling the "Intelligent Follow-up" requirement.
 * **Model Selection**: I utilized **Gemini 3.0 lite** for this POC to balance high-level intelligence with the low latency required for a real-time conversational interface.
